...
namespace Xamarin.Forms.MathDisplay
{
    ...
    {
        public static event StaticEventHandler<ChangedEventArgs<ViewLoc<Expression>>> CursorMoved;
        ...
        public static void Type(string str)
        {
            ...
            {
                list = new View[] { new Text(str) };// Reader.Render(Crunch.Machine.StringClassification.Simple(str));
            }
        ...
        public static bool MoveCursor(Expression parent, int i = 0)
        {
            ...
            ViewLoc<Expression> old = Cursor.Parent == null ? null : new ViewLoc<Expression>(Cursor.Parent, Index);

            Index = i;
            parent.Insert(Index, Cursor);
            //print.log("cursor moved", parent, Index);
            CursorMoved?.Invoke(new ChangedEventArgs<ViewLoc<Expression>>(old, new ViewLoc<Expression>(parent, Index)));
        ...
        public static int ChildCount(this Layout<View> layout) => layout.Children.Count - (SoftKeyboard.Cursor.Parent == layout).ToInt();
